...
class TestTimeEndpoint(unittest.TestCase):
    ...
    def _test_timezone(self, timezone):
        ...
        response = self.client.get(
            "/time", headers={"X-Timezone": timezone, "X-Location": self.location}
        )
        ...
        self.assertGreater(
            data[0], 1000000000000, "Timestamp should be in milliseconds"
        )
        ...
        self.assertTrue(
            data[2] is None or isinstance(data[2], int),
            "next_dst_change should be int or None",
        )
        self.assertTrue(
            data[3] is None or isinstance(data[3], int),
        ...
        if data[2] is not None:
            self.assertGreater(
                data[2], 1000000000000, "DST change should be in milliseconds"
            )
            self.assertIn(
                data[3], [3600, -3600], "DST offset change should be ±3600 seconds"
            )
    ...
    def test_invalid_timezone(self):
        ...
        response = self.client.get(
            "/time",
            headers={"X-Timezone": "Invalid/Timezone", "X-Location": self.location},
        )
        self.assertEqual(response.status_code, 404)

    def test_missing_location(self):
        """Test /time endpoint without location header (uses default NYC location)"""
        response = self.client.get("/time", headers={"X-Timezone": "America/New_York"})
